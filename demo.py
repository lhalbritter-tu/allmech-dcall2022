...
class InputTable(PseudoChangeable):
    """
    Implementation of Changeable which implements a Table of inputs
    """

    def __init__(self, n_cols=1, n_rows=2, headers=['Table'], filled_cells=['Cell1', 'Cell2']):
        self.head = [widgets.Text(
            value=header,
            description='',
            disabled=True
        ) for header in headers]
        self.content = [widgets.Text(
            value=cell,
            description='',
            disabled=False
        ) for cell in filled_cells]

        items = self.head + self.content
        widget = widgets.GridBox(
            items,
            layout=widgets.Layout(grid_template_columns=f'repeat({n_cols}, 300px)')
        )
        self.display = widget
        self.should_update = False
...
class SimpleModel(Model):
    ...
    def calculate(self):
        return "My params are: " + str(self.params)
